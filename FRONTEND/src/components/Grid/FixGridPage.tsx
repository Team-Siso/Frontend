...
const FixGridPage = ({ onPageChange }) => {
  ...
  useEffect(() => {
    ...
    // 루틴 셀 매핑 함수 (여러 셀을 채움)
    function mapRoutineToCells(startStr: string, endStr: string, dayIndex: number, sch: any): { [key: string]: RoutineInfo } {
  ...
  const handleAddRoutineClick = async () => {
    ...
    // Function to get the next occurrence of the selected dayOfWeek
    const getNextDateForDay = (dayOfWeek: string): Date => {
      ...
      const targetDay = daysOfWeekLocal.indexOf(dayOfWeek.trim());
      if (targetDay < 0) {
        alert("유효하지 않은 요일입니다.");
    ...
    function toIso(dayOfWeek: string, hhmm: string): string {
    ...
    const isoStart = toIso(routineForm.dayOfWeek, routineForm.startTime);
    const isoEnd = toIso(routineForm.dayOfWeek, routineForm.endTime);

    console.log("[FixGridPage] => about to call addRoutine with", {
      title: routineForm.name,
      day: routineForm.dayOfWeek,
      startTime: isoStart,
      endTime: isoEnd,
    });
